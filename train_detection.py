"""
表格检测训练脚本 (TrackA)
基于 Faster R-CNN 的文档级表格区域检测
用法: python train_detection.py [--resume] [--finetune PATH] [--epochs N]
"""
import os
import math
import time
...
def compute_fast_map(predictions, ground_truths, iou_threshold=0.5):
    """快速计算 mAP（表格检测：单类 table）"""
    all_classes = set()
...
def validate(model, val_loader, device, epoch, total_epochs):
    ...
    metrics = compute_fast_map(all_preds, all_gts, iou_threshold=0.5)
