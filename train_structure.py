"""
结构识别训练脚本 (TrackB)
用法: python train_structure.py [--resume] [--version N] [--backbone PATH] [--epochs N]
"""
import os
import time
import argparse
...
from config import (
    ...
    STRUCTURE_NUM_CLASSES, STRUCTURE_NMS_THRESH, init_dirs
)
import config
...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        train_metrics = train_one_epoch(model, train_loader, optimizer, device,
                                        epoch + 1, num_epochs, scaler=scaler, desc='Str Train')
        val_metrics = validate(model, val_loader, device, epoch + 1, num_epochs,
                               iou_threshold=0.3, use_dynamic=True, desc='Str Val')
    ...
    test_metrics = validate(model, test_loader, device, num_epochs, num_epochs,
                            iou_threshold=0.3, use_dynamic=True, desc='Test')
